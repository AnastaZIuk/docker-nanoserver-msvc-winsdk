...
COPY --from=buildtools ["C:/artifacts", "C:/Program Files (x86)/Windows Kits/10", "${BUILD_TOOLS_DIR}"]
...
ENV CMAKE_VERSION=${CMAKE_VERSION} CMAKE_URL=https://github.com/Kitware/CMake/releases/download/v${CMAKE_VERSION}/cmake-${CMAKE_VERSION}-windows-x86_64.zip CMAKE_DIR=${CMAKE_DIR} `
...
ARG PATH=C:\Windows\System32;C:\Windows
ENV PATH="${PATH};${CMAKE_DIR}\cmake-${CMAKE_VERSION}-windows-x86_64\bin;${PYTHON_DIR};${NINJA_DIR};${NASM_DIR}\nasm-${NASM_VERSION}"
